...
| task_id      | int(11)      | NO   | MUL | NULL
        ...
        sql = """
                   insert into spiderKey(url,keyWordNum,modifiedTime,startTime,taskId) values(%s,%s,%s,%s,%s);
               """
        cursor.execute(sql, (item.get('url'), item.get('keyWordNum'), item.get('modifiedTime'), item.get('startTime'),int(item.get("task_id"))))
