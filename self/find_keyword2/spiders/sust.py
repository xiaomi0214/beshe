    ...
    task_id=scrapy.Field()
        ...
        if find_num!=0:
            ...
            if modified_time!=None:
                print (modified_time)
                modifiedDate = datetime.datetime.strptime(modified_time.decode(), "%a, %d %b %Y %H:%M:%S %Z")
                modifiedDateStr = datetime.datetime.strftime(modifiedDate, "%Y-%m-%d %H:%M:%S")
                # content["modifiedTime"] =modifiedDateStr.decode('ascii')
                content["modifiedTime"] = modifiedDateStr
